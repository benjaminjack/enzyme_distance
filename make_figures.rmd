...
calc_ROC <- function(probabilities, known_truth, n=100, model.name=NULL)
  {
  ...
  # Threshold should really be from 0 to 1, but a value of exactly 0 or 1 leads
  # to TPRs and FPRs of greather than 1. Instead we use the min and max values
  # of our set of predicted probabilities.
  threshold <- seq(min(probabilities),max(probabilities),length.out = n)
  true_pos <- sapply(threshold,
                     function(x) sum(pos_probs>=x)/pos) # true pos. rate
  false_pos <- sapply(threshold,
...
calc_ROC_train_test <- function(data, model, train_fraction, n=100) {
...
subsample_validate <- function(data, model, train_fraction, replicates, n)
{
  reps <- data.frame(rep=1:replicates) # dummy data frame to iterate over
  reps %>% group_by(rep) %>% # iterate over all replicates
    do(calc_ROC_train_test(data, model, train_fraction,n)) %>% # run calc_ROC_train_test for each replicate
    ungroup() %>%
...
roc_df <- calc_ROC(glm.out$fitted.values, master.clean$ACTIVE_SITE, 100)
roc_plot <- ggplot(roc_df, aes(x=false_pos, y=true_pos)) + 
    geom_line() + 
    xlab("False Positive Rate") + 
    ylab("True Positive Rate") + xlim(0,1) + ylim(0,1)

pj7.roc_plot <- plot_grid(pj7, roc_plot, labels=c("a","b"), align = "h")
...
save_plot("figures/roc_curve.pdf", pj7.roc_plot, base_width = 8)
