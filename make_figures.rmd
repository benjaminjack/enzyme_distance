...
## All enzymes, no filtering

The following figures were generated using all enzymes and without removing interface residues. Please see the main text and supporting figures for figure captions.
...
facet_labeller <- function(var, value){
    ...
    if (var=="ntile.size") { 
        value[value=="1"] <- "Small Proteins"
        value[value=="2"] <- "Medium Proteins"
        value[value=="3"] <- "Large Proteins"
    }
    if (var=="null.group") {
        value[value=="1"] <- "RSA"
        value[value=="2"] <- "WCN"
        value[value=="3"] <- "WCN + RSA"
    }
    if (var=="rsa.group") { 
        value[value=="1"] <- "Core"
        value[value=="2"] <- "Intermediate"
        value[value=="3"] <- "Surface"
    }
    return(value)
}
...
master.clean <- group_by(master[complete.cases(master),],pdb,chain) %>% 
  mutate(multimer = str_trim(multimer))
...
# Interface residues ?
if (NO_INTERFACE == T) {
  master.clean <- filter(master.clean, RSA == RSA_mono)
}
...
master.preds <- select(master.clean, pdb, rate, shell, ends_with('.pred'), -dist_active.pred) %>%
  gather(pred.type, pred, -pdb, -shell) %>%
  mutate(null.group = get_null_group(pred.type), null.model = get_null(pred.type))

# Get empirical rates
empirical <- filter(master.preds, pred.type=='rate') %>% mutate(null.group = NULL, null.model = "empirical")
...
pj5 <- ggplot(filter(master.preds, pred.type != 'rate'), 
  ...
  xlim("0","1","2","3","4","5","6") +
  facet_grid(null.group ~ ., labeller=facet_labeller) +
...
target <- c('rate','wcnSC.RSA.dist_active.pred','wcnSC.RSA.pred')
pj20 <- ggplot(filter(master.preds, pred.type %in% target), 
  ...
  labs(color="Model", x="Shell", y="Rate") +
  scale_color_discrete(breaks=c("wcnSC.RSA.pred", "wcnSC.RSA.dist_active.pred", "rate"), labels=c("WCN + RSA", "WCN + RSA + d", "Empirical Rate")) +
...
pj21 <- pj20 %+% filter(master.res, pred.type %in% target) +
  ylab('Residual') +
  scale_color_discrete(breaks=target, labels=c('WCN + RSA', 'WCN + RSA + d'))
...
pj4 <- ggplot(master.clean, aes(x=factor(shell), y=rate)) +
  ...
  ylab('Rate') +
  xlim("0","1","2","3","4","5","6") +
  facet_grid( . ~ ntile.size, labeller = facet_labeller) +
  scale_fill_gradient(name="Residue Count", low="white", high="grey20", trans="sqrt") +
  panel_border()
...
pj6 <- ggplot(filter(master.res, pred.type != 'dist_active.res'), 
  ...
  stat_summary(fun.y=mean, geom="line") +
  xlim("0","1","2","3","4","5","6") +
  facet_grid(null.group ~ ntile.size, labeller=facet_labeller) +
  xlab('Shell') +
  ylab('Residual') +
...
pj8 <- pj6 + facet_grid(null.group ~ rsa.group, labeller=facet_labeller)
pj8
```

### Figure 5

```{r, echo=FALSE, message=FALSE, warning=FALSE, fig.height=12, fig.cap='Individual proteins from the data set in which distance is a strong predictor of rate but does not correlate with other structural metrics. (A) Correlations between different structural metrics and site-wise evolutionary rates. (B) Predicted site-wise evolutionary rates based on a generalized additive model. From top to bottom, structures are 1DHF, 1HKA, 1PJB, and 2ACY.'}
...
# master.lmcor.ex <- filter(master.lmcor, pdb == "1DHF" | pdb == "1HKA" | pdb == "1PJB" | pdb == "2ACY")
...
p8 <- ggplot(master.ex, aes(x=dist_active, y=gam_rate)) +
  ...
  ylab("Rate K") +
  facet_wrap(~ pdb, ncol=1)
p7.p8 <- plot_grid(p7, p8, ncol=2, align = 'h', rel_widths = c(1, 0.8))
p7.p8
```
...
# save_plot('fig1.pdf', p1.p2.pj3, base_width = 10, base_height = 8)
# save_plot('fig2.pdf', p6.pj20.pj21, base_width = 12, base_height = 6)
# save_plot('fig3.pdf', pj4.pj6, ncol=1, base_width = 10, base_height = 6.5)
# save_plot('fig4.pdf', pj8, base_width=8, base_height = 6.5)
# save_plot('fig5.pdf', p7.p8, ncol=2, nrow=1, base_height = 10, base_width = 4)
# save_plot('fig6.pdf', p9)
# save_plot('figS1.pdf', p11)
# save_plot('figS2.pdf', p5, base_width = 6, base_height = 4)
# save_plot('figS3.pdf', pj5, base_width = 6)
# save_plot('figS4.pdf', pj6, base_width = 8, base_height = 6.5)
